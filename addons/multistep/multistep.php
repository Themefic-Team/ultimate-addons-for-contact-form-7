			...
			'fields' => array(
				'placeholder_heading' => array(
					...
					'subtitle' => sprintf(
						...
						esc_html__( 'Create stunning multi-step forms with Contact Form 7. Ideal solution for long forms. See Demo %1s.', 'ultimate-addons-cf7' ),
				...
				'multistep_form_docs' => array(
					...
					'style' => 'success',
					'content' => sprintf(
						// Translators: %1$s is replaced with the link to the multi-step form documentation, %2$s is replaced with the link to the pro version documentation.
				...
				foreach ( $all_steps as $step ) {
					...
					<div class="steps-step"><a href="#step-<?php echo esc_attr( $step_id ); ?>" type="button"
							class="btn <?php if ( $step_id == 1 ) {
							  ...
							  echo wp_kses( apply_filters( 'uacf7_progressbar_step', esc_attr( $step_id ), $uacf7_multistep_use_step_labels, $content ), uacf7_custom_wp_kses_allow_tags() ); ?></a>
	...
	public function uacf7_post_meta_options_multistep_pro( $value, $post_id ) {
		...
		if ( $post_id != 0 ) {
			...
			if ( ! empty( array_filter( $all_steps ) ) ) {
				...
				// Add a translators comment to clarify the placeholder meaning
				/* translators: %d represents the step number in a multi-step form */
				$step_label = sprintf( __( 'Step %d', 'ultimate-addons-cf7' ), $step_count );
				foreach ( $all_steps as $step ) {
	...
	public function uacf7_properties( $properties, $cfform ) {
		if ( ! is_admin() || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
			...
			if ( $uacf7_multistep_is_multistep == true ) {
					...
					if ( $uacf7_progressbar_style == 'default' && ! empty( $uacf7_multistep_progressbar_title_color ) ) :
						...
						<style>
							.steps-form .steps-row .steps-step p {
								color:
									<?php echo esc_attr( $uacf7_multistep_progressbar_title_color ); ?>
								;
												...
												type="button" class="btn <?php if ( $step_id == 1 ) {
												  ...
												  if ( is_array( $step_name ) ) {
													  ...
													  if ( $uacf7_progressbar_style == 'style-7' ) {
														  ...
														  echo '<p>' . esc_html( substr( $content, 0, 20 ) ) . '</p>';
														  if ( isset( $multistep_meta[ 'desc_title_' . $step_name[ $step_count ] ] ) ) {
															  echo '<p>' . esc_html( substr( $multistep_meta[ 'desc_title_' . $step_name[ $step_count ] ], 0, 20 ) ) . '</p>';
														  }
														  echo '</div>';
														...
														<div class="uacf7-ms-skin8-step-wrapper">
															...
															<?php echo '<p class="uacf7-ms-skin8-total-step">' . esc_html( $content ) . '</p>'; ?>
														</div>
				...
				ob_start();
				echo wp_kses( apply_filters( 'uacf7_progressbar_html', $progressbar, $form, $cfform->id(), $all_steps ), uacf7_custom_wp_kses_allow_tags() );
				ob_start();
				?>
				...
				echo wp_kses( apply_filters( 'uacf7_form_html', $form_html ), uacf7_custom_wp_kses_allow_tags() );
