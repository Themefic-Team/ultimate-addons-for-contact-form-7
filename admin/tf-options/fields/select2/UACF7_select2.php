...
if ( ! class_exists( 'UACF7_select2' ) ) {
	class UACF7_select2 extends UACF7_Fields {
		...
		public function render() {
			...
			echo '<select name="' . esc_attr( $field_name ) . '" id="' . esc_attr( $tf_select2_unique_id ) . '" class=" tf-select-two '.esc_attr( $parent_class ).' " data-placeholder="' . wp_kses_post( $placeholder ) . '" ' . esc_attr( $multiple ) . ' '. wp_kses_post( $this->field_attributes() ) .'>';
