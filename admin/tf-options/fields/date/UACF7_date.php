...
if ( ! class_exists( 'UACF7_date' ) ) {
	class UACF7_date extends UACF7_Fields {
		...
		public function render() {
                ...
                <div class="tf-date-range">
                    <div class="tf-date-from">
                        ...
                        <div class="" style="position:relative;">
                            <input type="text" name="<?php echo esc_attr( $this->field_name() ); ?>[from]" placeholder="<?php echo wp_kses_post( $placeholder ) ?>" value="<?php echo esc_attr( $value['from'] ); ?>"
                    ...
                    <div class="tf-date-to">
                        ...
                        <div class="" style="position:relative;">
                            <input type="text" name="<?php echo esc_attr( $this->field_name() ); ?>[to]" placeholder="<?php echo wp_kses_post( $placeholder ) ?>" value="<?php echo esc_attr( $value['to'] ); ?>"
                                   class="flatpickr " data-format="<?php echo esc_attr( $format ); ?>" <?php echo wp_kses_post($this->field_attributes()) ?> data-min-date="<?php echo esc_attr( $minDate ); ?>"/>
                            <i class="fa-solid fa-calendar-days"></i>
                        </div>
                    </div>
			...
			<?php else: ?>
                <input type="text" name="<?php echo esc_attr( $this->field_name() ); ?>" placeholder="<?php echo wp_kses_post( $placeholder ) ?>" value="<?php echo esc_attr( $this->value ); ?>"
