...
if ( ! class_exists( 'UACF7_gallery' ) ) {
	class UACF7_gallery extends UACF7_Fields {
		...
		public function render() {
			...
			if(!empty($this->value)){
				echo '<a href="#" tf-field-name="' . esc_attr( $this->field_name() ) . '" class="tf-gallery-edit button button-primary button-large ' . str_replace(array("[","]","-"),"_",esc_attr( $this->field_name() ) ) . '">' . esc_html( "Edit Gallery","ultimate-addons-for-contact-form-7" ) . '</a><a href="#" tf-field-name="' . esc_attr( $this->field_name() ) . '" class="tf-gallery-remove button button-warning button-large ' . str_replace(array("[","]","-"),"_",esc_attr( $this->field_name() ) ) . '" style="display:inline-block">' . esc_html( "Clear","ultimate-addons-for-contact-form-7" ) . '</a>';
			}else{
