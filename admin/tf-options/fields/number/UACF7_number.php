...
if ( ! class_exists( 'UACF7_number' ) ) {
	class UACF7_number extends UACF7_Fields {
		...
		public function render() {
			...
			echo '<input type="' . esc_attr( $type ) . '" name="' . esc_attr( $this->field_name() ) . '" id="' . esc_attr( $this->field_name() ) . '" value="' . esc_attr($this->value) . '" ' . esc_attr( $placeholder ) . ' '. wp_kses_post( $this->field_attributes() ) .'/>';
		}
