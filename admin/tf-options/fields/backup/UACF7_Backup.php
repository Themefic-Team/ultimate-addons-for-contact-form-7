...
if ( ! class_exists( 'UACF7_Backup' ) ) {
    class UACF7_Backup extends UACF7_Fields {
        ...
        public function render() {
            ...
            echo '<hr>';
            echo '<textarea cols="50" rows="15" class="tf-exp-imp-field"  data-option="'.esc_attr( $this->settings_id ).'" name="tf_export_option" id="' . esc_attr( $this->field_name() ) . '"' . esc_attr($placeholder) . ' disabled '.wp_kses_post( $this->field_attributes()) .'>' . esc_html($current_settings) . '</textarea>';
